...
- [ ] **Fotos y descripciones reales de producto/"Quiénes somos"** — Joaquín las va subiendo/escribiendo directamente.
  ...
  - **2026-09-17, texto final de "Sobre nosotros" y "Nuestro proceso"** (Joaquín, confirmado): reemplaza la versión de texto que había desde el 2026-09-15 por la redacción definitiva que pasó Joaquín.
  - **Pendiente todavía**: `/nosotros` ("Nuestro proceso") sigue mostrando el banner placeholder genérico (`salmon-recien-salido-nuestro-proceso.jpg`) — el texto de esa sección ya está definitivo, pero la foto todavía no. ("Sobre nosotros" ya tiene foto real, `quienes-somos-pablo-gellona.jpeg`.)
- [x] **Responsive / versión mobile** — el nav con menú hamburguesa (`app/javascript/controllers/nav_menu_controller.js`) ya está commiteado. Durante la sesión del 2026-09-15 se verificó con screenshots el collage de "Nuestros productos" y la grilla de catálogo en mobile (390px) — se ven bien. Sigue faltando una pasada de Joaquín en su teléfono real para el resto de páginas (checkout, carrito, perfil, etc.).
  - **2026-09-17, cajón mobile y navbar retocados** (Joaquín, confirmado con capturas): el cajón lateral ahora tiene fondo blanco de punta a punta (antes dejaba ver una franja navy del nav arriba, donde está la X de cerrar). Se agregó un ícono de carrito propio junto a la hamburguesa (antes solo estaba dentro del cajón), con el mismo contador de `carrito_count` que ya se usaba en el resto del sitio. En desktop volvió el texto "Carrito" junto al ícono (se había sacado el 2026-09-17 más temprano en `a35992c`) — esto obligó a subir el breakpoint del nav de 1100px a 1250px de nuevo, porque "Iniciar Sesión" se cortaba en dos líneas en ese rango con el texto nuevo.
  - **2026-09-17, catálogo "Nuestros productos" rediseñado en mobile** (Joaquín, confirmado con capturas): barra de filtros sticky (queda pegada debajo del nav al scrollear, en mobile y desktop); "Ordenar por" + "Precio" se fusionaron en un botón "Filtrar por" con panel desplegable, dejando el buscador como único campo grande de la fila. El grid de productos se probó primero a 1 columna grande y después volvió a 2 columnas (a pedido de Joaquín) con el nombre a un font-size más chico (tope 2 líneas) y el peso como subtítulo separado, gris, sin partirse nunca en dos líneas — así las cards de una fila quedan parejas en alto. Se sacó el botón "Agregar al carrito" de la vista general en mobile (se agrega desde la ficha). En la ficha de producto, el stepper de cantidad + "Agregar al carrito" pasan a una barra fija abajo de la pantalla en mobile, siempre visible mientras se scrollea la descripción/reseñas.
  - **2026-09-17, más ajustes mobile de ficha de producto, Quiénes somos, modal y carrito** (Joaquín, confirmado con capturas): en la ficha, el título ahora reordena a título+precio antes que la foto (con `display:contents`+`order`, sin tocar el HTML ni el layout de escritorio), se agregó el peso como subtítulo y el precio quedó sin negrita, y la barra fija de cantidad+agregar quedó con altura pareja (48px) entre sus elementos. En "Quiénes somos", título+texto pasan a ir antes que la foto en mobile (mismo truco de `order`) y los párrafos de ambas secciones se acortaron a uno solo cada uno. El modal "Producto añadido al carrito" se corrigió: pasó de centrado puro (que en mobile real podía quedar con el título tapado por el nav, por el toolbar dinámico del navegador) a anclado con margen superior fijo. El carrito pasó de una columna apilada a 2 columnas lado a lado (productos | resumen sticky) en mobile.
